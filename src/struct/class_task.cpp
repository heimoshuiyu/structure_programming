...
struct Student { // each student will have three elements: name, major, score
  ...
  float score;
  char major[10];
}; // Please always define the structure outside the main function, so that each
...
int main() {
  ...
  */
  sort(stu, num); // call the sort function to sort the students by name. Notice
                  // that we use the array name as the input
  printf("Sorted Alphabetically\n");
  printList(stu, num);

  printf("\n");

  sort_score(stu, num); // call the sort function to sort the students by name. Notice
                  // that we use the array name as the input
  printf("Sorted Alphabetically\n");
  printList(stu, num);
